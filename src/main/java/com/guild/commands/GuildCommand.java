...
public class GuildCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleEconomy(Player player, String[] args) {
        plugin.getGuildService().getPlayerGuildAsync(player.getUniqueId()).thenAccept(guild -> {
            if (guild == null) {
                String message = plugin.getConfigManager().getMessagesConfig().getString("economy.no-guild", "&cVocê ainda não entrou em nenhuma guilda!");
                player.sendMessage(ColorUtils.colorize(message));
                return;
            }
            String message = plugin.getConfigManager().getMessagesConfig().getString("economy.info", "&6Informações da Economia da Guilda");
            player.sendMessage(ColorUtils.colorize(message));
            String balanceMessage = plugin.getConfigManager().getMessagesConfig().getString("economy.balance", "&7Saldo Atual: &e{balance}")
                .replace("{balance}", plugin.getEconomyManager().format(guild.getBalance()));
            player.sendMessage(ColorUtils.colorize(balanceMessage));
            String levelMessage = plugin.getConfigManager().getMessagesConfig().getString("economy.level", "&7Nível Atual: &e{level}")
                .replace("{level}", String.valueOf(guild.getLevel()));
            player.sendMessage(ColorUtils.colorize(levelMessage));
            String maxMembersMessage = plugin.getConfigManager().getMessagesConfig().getString("economy.max-members", "&7Máx. Membros: &e{max_members}")
    ...
    private void handleDeposit(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage(ColorUtils.colorize("&cUso: /guild deposit <Quantia>"));
            return;
        }
        double amount;
        try {
            ...
            player.sendMessage(ColorUtils.colorize("&cFormato de quantia inválido!"));
            return;
        }
        if (amount <= 0) {
            player.sendMessage(ColorUtils.colorize("&cQuantia deve ser maior que 0!"));
            return;
        }
        plugin.getGuildService().getPlayerGuildAsync(player.getUniqueId()).thenAccept(guild -> {
            if (guild == null) {
            ...
            if (!plugin.getEconomyManager().hasBalance(player, amount)) {
                String message = plugin.getConfigManager().getMessagesConfig().getString("economy.insufficient-balance", "&cSeu saldo é insuficiente!");
                player.sendMessage(ColorUtils.colorize(message));
                return;
            }
            ...
            plugin.getGuildService().updateGuildBalanceAsync(guild.getId(), guild.getBalance() + amount).thenAccept(success -> {
                if (success) {
                    String message = plugin.getConfigManager().getMessagesConfig().getString("economy.deposit-success", "&aDepósito de &e{amount} &ana guilda realizado com sucesso!")
                        .replace("{amount}", plugin.getEconomyManager().format(amount));
                    player.sendMessage(ColorUtils.colorize(message));
                } else {
                    plugin.getEconomyManager().deposit(player, amount);
                    String message = plugin.getConfigManager().getMessagesConfig().getString("economy.deposit-failed", "&cFalha no depósito!");
                    player.sendMessage(ColorUtils.colorize(message));
                }
            });
        });
    ...
    private void handleWithdraw(Player player, String[] args) {
        if (args.length < 2) {
            player.sendMessage(ColorUtils.colorize("&cUso: /guild withdraw <Quantia>"));
            return;
        }
        double amount;
        try {
        ...
        plugin.getGuildService().getPlayerGuildAsync(player.getUniqueId()).thenAccept(guild -> {
            ...
            if (guild.getBalance() < amount) {
                String message = plugin.getConfigManager().getMessagesConfig().getString("economy.guild-insufficient-balance", "&cSaldo da guilda insuficiente!");
                player.sendMessage(ColorUtils.colorize(message));
                return;
            }
            plugin.getGuildService().getGuildMemberAsync(guild.getId(), player.getUniqueId()).thenAccept(member -> {
                if (member == null || member.getRole() != GuildMember.Role.LEADER) {
                    String message = plugin.getConfigManager().getMessagesConfig().getString("economy.leader-only", "&cApenas o líder da guilda pode sacar!");
                    player.sendMessage(ColorUtils.colorize(message));
                    return;
                }
                plugin.getGuildService().updateGuildBalanceAsync(guild.getId(), guild.getBalance() - amount).thenAccept(success -> {
                    if (success) {
                        plugin.getEconomyManager().deposit(player, amount);
                        String message = plugin.getConfigManager().getMessagesConfig().getString("economy.withdraw-success", "&aSaque de &e{amount} &ada guilda realizado com sucesso!")
                            .replace("{amount}", plugin.getEconomyManager().format(amount));
                        player.sendMessage(ColorUtils.colorize(message));
                    } else {
                        String message = plugin.getConfigManager().getMessagesConfig().getString("economy.withdraw-failed", "&cFalha no saque!");
                        player.sendMessage(ColorUtils.colorize(message));
                    }
                });
            });
    ...
    private void handleTransfer(Player player, String[] args) {
        if (args.length < 3) {
            player.sendMessage(ColorUtils.colorize("&cUso: /guild transfer <Guilda> <Quantia>"));
            return;
        }
        ...
        plugin.getGuildService().getPlayerGuildAsync(player.getUniqueId()).thenAccept(sourceGuild -> {
            ...
            plugin.getGuildService().getGuildMemberAsync(sourceGuild.getId(), player.getUniqueId()).thenAccept(member -> {
                if (member == null || member.getRole() != GuildMember.Role.LEADER) {
                    String message = plugin.getConfigManager().getMessagesConfig().getString("economy.leader-only", "&cApenas o líder da guilda pode transferir!");
                    player.sendMessage(ColorUtils.colorize(message));
                    return;
                }
                ...
                plugin.getGuildService().getGuildByNameAsync(targetGuildName).thenAccept(targetGuild -> {
                    if (targetGuild == null) {
                        String message = plugin.getConfigManager().getMessagesConfig().getString("economy.target-guild-not-found", "&cGuilda alvo não existe!");
                        player.sendMessage(ColorUtils.colorize(message));
                        return;
                    }
                    if (sourceGuild.getId() == targetGuild.getId()) {
                        String message = plugin.getConfigManager().getMessagesConfig().getString("economy.cannot-transfer-to-self", "&cNão é possível transferir para sua própria guilda!");
                        player.sendMessage(ColorUtils.colorize(message));
                        return;
                    }
                    plugin.getGuildService().updateGuildBalanceAsync(sourceGuild.getId(), sourceGuild.getBalance() - amount).thenAccept(success1 -> {
                        if (success1) {
                            plugin.getGuildService().updateGuildBalanceAsync(targetGuild.getId(), targetGuild.getBalance() + amount).thenAccept(success2 -> {
                                if (success2) {
                                    String message = plugin.getConfigManager().getMessagesConfig().getString("economy.transfer-success", "&aTransferência de &e{amount} &apara a guilda &e{target} &arealizada com sucesso!")
                                    ...
                                    String message = plugin.getConfigManager().getMessagesConfig().getString("economy.transfer-failed", "&cFalha na transferência!");
                                    player.sendMessage(ColorUtils.colorize(message));
                                }
                            });
                        } else {
